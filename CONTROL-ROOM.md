...
| Phase 5: case-study integration content | DRAFTED. `src/data/decisionosCase.controlroom-draft.js` (unimported): additive `controlRoom` section (five patterns with screenshot slots + live prototype link) plus hero/whereItStands patches and the prototypeUrl flip. Compiles clean. OPEN FOR FARAZ: (a) the narrative fork, extend vs replace the decision-brief framing (draft written for EXTEND); (b) voice pass on the copy; (c) capture merge-day screenshots into public/decisionos-shots/ at the S() paths (cr-01..cr-05). |
...
| Study aid / interview prep | DONE. `CRUX-EXPLAINER.md` at repo root (~13k words, 11 sections: what/why/how/agents/benefits/use-cases/differentiation/**security+safety**/build-process/hard-recruiter-questions/glossary, plus a **References** accordion of verified sources), all plain spoon-fed language, honest posture, no em-dashes. Security section built via an adversarial "skeptical CISO" workflow (draws the infra-vs-design line, hands prompt-injection/log-tamper/threat-model to engineering, never overclaims). References web-verified live and curated to canonical sources (agentexperience.ax, Microsoft HAX, Google PAIR, Shape of AI, NN/g, Figma hiring data). Rendered study-guide page at `public/decisionos/explainer.html` (stripped on deploy) and published as a claude.ai Artifact. |
| Merge into main site | DONE IN CODE, NOT DEPLOYED (Faraz: "Full merge without deploy", 2026-07-06). content.js decisionos `live:true` (card live on Projects, real cr-01 cover, live-prototype badge). decisionosCase.js REBUILT around the Control Room (fork: "rebuild around the Control Room"): working prototype is the spine, embedded live via `prototypeUrl:/decisionos/` iframe + fullscreen link; five trust patterns as "key screens" (cr-01..05 captured via `scripts/shoot-decisionos.mjs`); the decision-brief demoted to "Exploration 01-04" wireframes. Garnish moved to `public/` root. Verified: both files compile, case study + Projects render clean (no console errors, no overflow, all images decode), `CI=false npm run build` green, react-snap 11/11 including /case/decisionos. NOT pushed to gh-pages. Visual polish deferred per Faraz. |
| Deploy (when Faraz says go) | On deploy, do NOT strip build/decisionos anymore (the prototype ships now). Instead strip only the private study-guide files: `rm -f frontend/build/decisionos/explainer.html frontend/build/decisionos/commentary-demo.html`. Then normal ritual: verify hashed bundle 200 on live domain. |
