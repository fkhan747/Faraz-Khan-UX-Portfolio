...
export const finvista = {
  ...
  v4: {
    title: ["FinVista: Architecting an", "Assisted Lending Operating System at Scale"],
    tags: "Enterprise Fintech · Native Android · Assisted Lending · Design System · Field Operations",
    deck: "Loan applications used to take days and ran on paper. I rebuilt the whole thing as an eleven-step Android journey for one of India's largest NBFCs.",
    ...
    pair: [
      { h: "The Challenge", ps: ["The same Android phone has to work for two different people. A field agent gets through fifteen to twenty applications a day on a cheap handset, usually standing in a stockyard on a 4G signal that keeps dropping. Then they hand that same phone to the borrower for the photo, the consents, the OTP and the signature. There are over forty fields to fill in for one application, and the whole thing had to work for six lending products without building six separate apps."] },
      { h: "Our Solution", ps: ["The app has two states and switches between them. When the agent is using it, it is dense and quick to fill in. When they hand the phone over, the type gets bigger, the language gets plainer, and the internal notes come off the screen. The journey is eleven steps with one job on each screen, and every field is saved to the phone as you go, so a dropped signal does not cost you anything. Underneath is a design system of over a hundred components that the other five loan products reused as it was."] },
    ],
    lead: { items: [
    ...
    ], cap: "The first two screens are what the agent sees. The third is what the borrower sees once the phone has been handed over." },
    kpis: [["60%", "faster loan processing"], ["11", "steps, one job per screen"], ["6", "verticals on one system"], ["86", "SUS, up from 58"]],

    sections: [
      {
        n: "01", h: "Operating Context and Enterprise Constraints",
        p: "This was never going to be used in an office. The people using it are standing in vehicle stockyards and dealership lots, on company phones that cost about ten thousand rupees. Three things shaped the architecture:",
        cards: [
          ["Device and network diversity", "The phones were budget Android handsets with very little memory, and the 4G signal dropped constantly in outdoor stockyards and busy dealerships."],
          ["Information overload", "A single loan application needs more than forty separate pieces of information."],
          ["The multi-vertical mandate", "We built it for two-wheeler loans first, but consumer durables, personal loans, used cars, tractors and three-wheelers all had to run on the same system."],
        ],
      },
      {
        h: "The journey, screen by screen",
        p: "These are the eleven steps in the order the agent works through them. On two of them the phone gets turned around: the credit bureau consent, and the key facts statement where the borrower signs.",
        film: [
        ...
        filmHint: "Scroll sideways, and tap any screen to open it.",
      },
      {
        n: "02", h: "The Core Innovation: Two Readers, One Screen",
        p: "The thing that unlocked it was realising two completely different people were sharing one device. The app switches between two states, and three rules govern how it behaves:",
        cards: [
          ["A · High-throughput agent mode", "The agent is doing fifteen to twenty of these a day, so this state is built for speed. The fields are packed in tight and the number pads are dedicated."],
          ["B · Sandboxed borrower handoff", "The agent taps to hand over and the screen changes. Type goes up to 18 point, the dealership notes and risk scores are hidden, the wording gets simpler, and there is one decision to make."],
          ["C · Fail-safe state preservation", "Every field is written to the phone as soon as it is filled in, so losing signal halfway round a showroom does not lose any of the work."],
        ],
        media: [{ items: [{ src: "/finvista/m3/employment.png", alt: "State A, agent mode" }, { src: "/finvista/m3/consent.png", alt: "State B, borrower handoff" }], cap: "On the left the agent is entering data. On the right the phone has been handed over for consent." }],
      },
      {
        n: "03", h: "Architecture: Why the 11\u2011Step Modular Stepper Won",
        p: "We tried long scrolling forms and we tried accordions, and both fell over on the cheap phones. The viewport jumped around unpredictably, and forty fields on one screen was more than anyone could hold in their head. So the journey became eleven separate steps with one job on each screen. It takes more taps that way, but nobody has to work out what to do next, and people finish faster.",
        stepsTitle: "The eleven steps",
        steps: ["Basic Details", "PAN Check", "Assisted KYC", "Income Profiling", "Asset Selection", "Loan Configurator", "Bank Validate", "E-Mandate Setup", "Down Payment", "Delivery Order", "Disbursement"],
        handoffSteps: [3, 8],
        media: [{ items: [{ src: "/finvista/svg-13.svg", alt: "Layout exploration: long scroll, accordion, stepped wizard" }], cap: "The layout options we tried: the long scroll, the accordion, and the stepped wizard that shipped." }],
      },
      {
        n: "04", h: "Usability Testing and Iteration",
        p: "I ran five rounds of moderated testing before we committed to the architecture, using high-fidelity Figma prototypes mirrored onto the same budget Android phones people would be using.",
        ...
        friction: [
          ["The long-scroll fatigue", "Scrolling through forty fields on one screen wore people out. They made more mistakes, and the viewport kept jumping on the cheaper phones.", "We split it into the eleven-step flow, so there is one task on the screen at a time."],
          ["Spatial disorientation", "Once it was split into steps, agents lost track of where they were, and the longer compliance checks made people anxious.", "We added two progress indicators side by side, one for how far through the whole journey you are and one for which step you are on."],
        ],
        media: [{ items: [{ src: "/finvista/svg-15.svg", alt: "Progress exploration: top bar, vertical stepper, ring" }, { src: "/finvista/m3/vehicle.png", alt: "Shipped: the dual progress indicator" }], cap: "The progress options we looked at on the left. On the right, what shipped: the journey percentage and the step count together, on every screen." }],
      },
      {
        n: "05", h: "Scaling Across the Business",
        p: "The architecture also had to handle tractors, used cars and consumer durables. A design system of more than a hundred native Android components carried the same eleven-step logic across five more loan products.",
        cards: [
          ["100+ components", "Built on tokenised Material 3 foundations, covering colour, type, spacing, form controls, cards and sheets."],
          ["Same steps, different proof", "Tractor loans replaced the salary fields with land revenue records. The steps themselves stayed exactly the same."],
          ["Express 4-step checkout", "Existing customers taking a durables or personal loan get a pre-approved journey, which brings the eleven steps down to four."],
        ],
        media: [{ items: [{ src: "/finvista/m3/system.png", alt: "The FinVista design system", sheet: true }], cap: "The design system, covering colour roles, the type scale, spacing, controls, status and the handoff screens." }],
      },
      {
        n: "06", h: "Strategic Retrospective",
        sub: "Field learnings",
        numbered: [
          ["What looks good in Figma can be unreadable outdoors", "The 1px light-grey dividers looked refined on my screen and disappeared completely under midday sun in a stockyard. I changed the tokens to higher contrast, heavier borders, and a 16sp minimum on every field."],
          ["Plan for the network being slow", "Credit bureau and bank checks regularly took four to eight seconds. Agents assumed nothing had happened and tapped submit again, which fired duplicate calls. Loading skeletons and debounced buttons stopped that."],
        ],
      },
    ],
  },
