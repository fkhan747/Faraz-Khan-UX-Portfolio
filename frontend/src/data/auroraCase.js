...
export const aurora = {
  ...
  title: "Aurora",
  subtitle:
    "A marketing campaign platform's experience transformation. Redesigning the Recurring Batch Journey configurator and embedding an in-canvas AI composer (Helio AI) during a parallel AngularJS-to-Angular migration — cutting clicks for campaign setup 3:1 and aligning the product with Helio's unified brand language.",
  hero: {
    stats: [
  ...
  overview: {
    ...
    intro2:
      "The legacy product was engineer-led — functional but inconsistent, dense, and visually fragmented. Marketers managing recurring campaigns navigated cluttered configuration panels, and copywriters had no AI assistance inside the composer, forcing context-switches to ChatGPT and back.",
  ...
  problem: {
    ...
    challenges: [
      { t: "No Cohesive Brand Language", d: "Aurora's UI looked completely fragmented with no consistency. Existing customers had already raised tickets for unclear flow interaction and confusing UI — issues that compounded as Helio's parent brand introduced a unified design language." },
      ...
      { t: "Inconsistent User Experience", d: "Similar functions had different flows and approaches with different UI elements, making the experience confusing. Without a unified design system, every new feature looked and behaved slightly differently — frustrating customers and eroding brand trust." },
    ...
    scopeIntro:
      "We defined clear boundaries to deliver maximum impact within the AngularJS-to-Angular migration window, focusing on the two highest-friction surfaces — recurring journey configuration and the email composer — while building a reusable design system in parallel.",
    inScope: [
    ...
    hypothesis:
      "We believe that consolidating recurring journey setup into a three-tab guided flow (Type → Content → Properties) and embedding Helio AI directly in the composer — backed by a unified Figma design system — will reduce clicks, prevent scheduling errors, and align Aurora with Helio's brand experience.",
    hypothesisBullets: [
  ...
  research: {
    ...
    auditTitle: "UX Audit",
    auditIntro:
      "A systematic audit of the existing Recurring Batch Journey and message composer revealed systemic usability issues across setup, scheduling comprehension, and content authoring — issues compounded by the legacy AngularJS codebase's inflexibility.",
    auditFindings: [
      ...
      { t: "Composer Friction", d: "The text block had no formatting helpers, no AI, no variation tools, and no way to test tone — pure freeform. Subject-line authoring offered no support beyond a 50-character input." },
    ...
    findings: [
      ...
      { t: "Composer Workarounds", d: "64% of CRM managers admitted to drafting copy in ChatGPT or Google Docs first, then pasting back — \"the tool is for sending, ChatGPT is for writing\" (Participant 7)." },
      { t: "Failed Test Queries", d: "Marketers wanted alerts from lists, journeys, and tasks but the dashboard only showed failed test queries — useful for QA, useless for daily marketing operations. Dashboard utility was a strong unmet need." },
      { t: "Tooltip Truncation", d: "The journey name is important but gets truncated — users needed to hover to see the full name in a tooltip. Marketers used long names to specify a journey's purpose, and the truncation defeated that workflow entirely." },
      { t: "Yesterday-Only Dashboard", d: "Dashboards only showed journeys executed yesterday. Marketers needed visibility into past FY quarters to assess yearly performance — a frequent ask that drove customers to export data and analyze elsewhere." },
  ...
  insights: {
    ...
    designGoals: [
      ...
      { t: "Demystify Cadence", d: "Always show a plain-English summary — \"The content will be sent weekly on Wednesday at 09:00 AM in Europe/Paris\" — so users never have to interpret raw settings." },
      { t: "Bring AI to the Canvas", d: "Surface Helio AI from the text block, subject line, and preheader — never as a separate page or detour. Quick actions and 5-option variation pickers." },
    ...
    keyInsight:
      "Power CRM managers ran 4–6 campaigns per week but spent ~22 minutes on each — 14 of those minutes on scheduling and copywriting, the two areas with zero in-product support.",
  },
  ...
  design: {
    ...
    finalScreens: [
      { src: A("01_RBJ_Type_Tab_Basic_Fields.jpg"), tag: "Hi-fi · RBJ · 01", title: "RBJ · Type Tab — Basic Fields", desc: "Entry point of the 3-tab creation flow. Name, folder path, and asset selection only — basic fields surfaced first, advanced fields tucked behind the \"All Fields\" toggle." },
      ...
      { src: A("07_Composer_Empty_AI.jpg"), tag: "Hi-fi · Helio AI · 07", title: "Composer · Helio AI Empty State", desc: "\"How can I help you today?\" — single text input, no clutter. The panel waits for the author to lead rather than presenting an option salad up front." },
      ...
      { src: A("11_Composer_Blocks_Library_Panel.jpg"), tag: "Hi-fi · Helio AI · 11", title: "Composer · Blocks Library Panel", desc: "The right-rail Blocks library — Rows (1/2/3/4 columns), Content (Text, Image, Spacer, Divider, etc.). Style / Variables / Links / Attachments tabs above for deeper control. Lets writers compose without leaving the canvas." },
    ],
  },
  ...
  designSystem: {
    ...
    intro:
      "In parallel with the Recurring Batch Journey and Helio AI work, I built the Aurora Engage UI Kit in Figma — a reusable design system that aligned Aurora with Helio's parent brand language. Tokens, components, and patterns were linked directly to Azure DevOps user stories so engineering could pick them up without ambiguity. The kit is the source of truth for every screen shown in section 05.",
    colors: [
