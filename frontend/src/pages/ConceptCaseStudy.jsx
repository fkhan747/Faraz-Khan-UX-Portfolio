...
import { ArrowLeft, Mail, Check, X, Sparkles, Maximize2 } from "lucide-react";
...
const SectionWrap = ({ children, className = "", ...rest }) => (
  <section className={`py-16 md:py-20 border-t border-white/10 ${className}`} {...rest}>
  ...
  return (
    <article data-testid={`${r.slug}-case-study`} className="pb-24">
      ...
      <SectionWrap data-testid="section-overview">
        ...
        {r.overview.badge && (
          <Reveal as="p" className="inline-block text-[11px] font-mono uppercase tracking-[0.2em] text-[#F5379B] border border-[#F5379B]/40 rounded-full px-3 py-1 mb-6">{r.overview.badge}</Reveal>
        )}
        ...
        <Reveal as="p" delay={0.08} className="text-lg md:text-xl leading-relaxed max-w-4xl text-[#F4F3FA]">{r.overview.tldr}</Reveal>
      </SectionWrap>
      )}
      ...
      <SectionWrap data-testid="section-concept">
        ...
        {hyp.positioning && (
          <Reveal as="p" className="text-lg md:text-xl leading-relaxed max-w-5xl text-[#F4F3FA] mb-10">{hyp.positioning}</Reveal>
        )}
        {(hyp.isItList || hyp.isNotList) && (
          <div className="grid md:grid-cols-2 gap-5">
            {hyp.isItList && (
              <Reveal className="dark-card rounded-3xl p-7">
                <p className="text-[11px] font-mono uppercase tracking-[0.25em] text-[#F5379B] mb-4">what it is</p>
                <ul className="space-y-3">
                  {hyp.isItList.map((x) => (<li key={x} className="flex items-start gap-3 text-base"><Check className="flex-shrink-0 mt-1 text-[#F5379B]" size={16} /><span>{x}</span></li>))}
                </ul>
              </Reveal>
            )}
            {hyp.isNotList && (
      ...
      <SectionWrap data-testid="section-prototype">
        ...
        {r.prototypeUrl ? (
          <>
            {design.finalIntro && (
              <Reveal as="p" className="text-lg md:text-xl leading-relaxed max-w-5xl text-[#F4F3FA] mb-6">{design.finalIntro}</Reveal>
            )}
            <div className="mb-4 flex flex-wrap items-center gap-3">
              ...
              <span className="text-xs font-mono uppercase tracking-widest text-white">{design.prototypeHint || "click anything. it's the real prototype, not a mock-up."}</span>
            </div>
        ...
        {design.finalScreens && design.finalScreens.length > 0 && (
          <>
            ...
            <Reveal as="p" delay={0.06} className="text-sm text-[#A29CB4] mb-6">The prototype above is the real thing; these are highlights.</Reveal>
            <div className="grid md:grid-cols-2 gap-5">
              {design.finalScreens.map((s, i) => (
                <Reveal key={s.title} as="figure" delay={(i % 2) * 0.05} className="rounded-2xl dark-card overflow-hidden">
                  ...
                  <figcaption className="p-5">
                    ...
                    <p className="mt-1.5 text-sm leading-relaxed text-[#F4F3FA]/80">{s.desc}</p>
                  </figcaption>
                </Reveal>
              ))}
            </div>
      ...
      <SectionWrap data-testid="section-ai-layer">
        ...
        {ai.intro && (
          <Reveal as="p" delay={0.08} className="text-base md:text-lg leading-relaxed max-w-4xl text-[#F4F3FA] mb-10">{ai.intro}</Reveal>
        )}
        <div className="grid md:grid-cols-2 gap-5">
          {ai.patterns.map((p, i) => (
            <Reveal key={p.id || p.t} delay={(i % 2) * 0.06} className="dark-card rounded-3xl p-7">
              <div className="flex items-center gap-3 mb-2">
                {p.id && <span className="text-[10px] font-mono uppercase tracking-widest text-[#F5379B]">{p.id}</span>}
                <h3 className="font-display text-lg font-black" style={p.featured ? gradText : undefined}>{p.t}</h3>
              </div>
              <p className="text-sm leading-relaxed text-[#F4F3FA]/85">{p.d}</p>
            </Reveal>
          ))}
        </div>
      </SectionWrap>
      ...
      <SectionWrap data-testid="section-where-it-stands">
        ...
        {stands.intro && (
          <Reveal as="p" delay={0.08} className="text-lg md:text-xl leading-relaxed max-w-4xl text-[#F4F3FA] mb-10">{stands.intro}</Reveal>
        )}
        <div className="grid md:grid-cols-2 gap-5">
          <Reveal className="dark-card rounded-3xl p-7">
            <p className="text-[11px] font-mono uppercase tracking-[0.25em] text-[#F5379B] mb-4">{stands.doneTitle || "what's done"}</p>
            <ul className="space-y-3">
              {stands.done.map((x) => (<li key={x} className="flex items-start gap-3 text-sm md:text-base"><Check className="flex-shrink-0 mt-1 text-[#F5379B]" size={16} /><span>{x}</span></li>))}
            </ul>
          </Reveal>
          {stands.targets && (
            <Reveal delay={0.06} className="dark-card rounded-3xl p-7">
              ...
              <ul className="space-y-4">
                {stands.targets.map((t) => (
                  <li key={t.t}>
                    <p className="font-display text-base font-bold text-[#F4F3FA]">{t.t}</p>
                    <p className="mt-1 text-sm leading-relaxed text-[#F4F3FA]/80">{t.d}</p>
                  </li>
                ))}
              </ul>
            </Reveal>
        ...
        <Reveal as="p" delay={0.1} className="mt-8 text-sm md:text-base text-[#A29CB4] max-w-3xl">
      ...
      <SectionWrap className="text-center">
        ...
        <Reveal as="p" delay={0.08} className="text-lg text-[#A29CB4] mb-8">{r.title} is a self-initiated concept I keep iterating on. If you&apos;d like to talk through the thinking, or where it goes next, I&apos;d love to connect.</Reveal>
