  ...
  return (
    <div data-testid="projects-page">
      <section className="pt-12 pb-10">
        <Container>
        <p className="text-[11px] font-mono uppercase tracking-[0.25em] text-[#E94B1F] mb-6">selected work · 2023 — 2025</p>
      ...
      <section className="pb-24" data-testid="client-work-section">
        <Container>
        <p className="text-[11px] font-mono uppercase tracking-[0.25em] text-[#E94B1F] mb-6">client work</p>
        <Grid>
          {projects.map((p, i) => (
            <Link
              key={p.slug}
              to={`/case/${p.slug}`}
              className={`col-span-12 md:col-span-6 group block rounded-3xl overflow-hidden bg-[#F7F1DA] lift rise rise-${(i % 6) + 1}`}
              ...
              <div className="px-6 md:px-7 py-5 flex items-start justify-between gap-4">
                <div>
                  <div className="flex items-baseline gap-3 mb-1">
                    ...
                    <span className="text-[10px] font-mono uppercase tracking-widest text-[#5A5A5A]">{p.year}</span>
                  </div>
                  <p className="text-sm md:text-base text-[#5A5A5A]">{p.subtitle}</p>
                </div>
      ...
      <section className="pb-24" data-testid="concepts-section">
        <Container>
        <p className="inline-flex items-center gap-2 text-[11px] font-mono uppercase tracking-[0.25em] text-[#E94B1F] mb-6">
        ...
        <p className="max-w-2xl text-lg md:text-xl text-[#1a1a1a] mb-12">
          ...
          {concepts.map((c, i) => {
            const inner = (
              <>
                <div
                  ...
                  style={{ background: `linear-gradient(140deg, ${c.accent}, #0A0A0A 118%)` }}
                >
            ...
            return c.live ? (
              <Link
                ...
                className={`col-span-12 md:col-span-4 group block rounded-3xl overflow-hidden bg-[#0A0A0A] border border-white/10 lift rise rise-${(i % 6) + 1}`}
              >
                {inner}
              </Link>
            ) : (
              <div
                key={c.slug}
                data-testid={`concept-card-${c.slug}`}
                className="col-span-12 md:col-span-4 rounded-3xl overflow-hidden bg-[#0A0A0A] border border-white/10"
              >
                {inner}
              </div>
            );
