...
import { Play, Cpu, Check, ShieldCheck, GitBranch, UserCheck, Globe, Lock } from "lucide-react";
import Seo from "../components/Seo";
import CaseTopBar from "../components/CaseTopBar";
import { DECK_CSS } from "./pilots/caseDeck";
import { Head, Thanks } from "./pilots/deckParts";
...
/* The demo paints these through inline styles, which the theme-light remap
   cannot reach, so they carry the paper-safe values directly. #F5379B is
   3.04:1 on the light ground and #075EFD is the old dark-shell blue. */
const PINK = "#C71E73", BLUE = "#1B5FB3";

/* Same treatment as the other case studies: the slide-band deck system, its
   own accent, CaseTopBar, and the shared Thanks closing. The blue is the one
   the AI Lab uses, since that is where this page is reached from. */
const ACCENT = "#1B5FB3";
const NEXT_WORK = [
...
const SC_STATUS = {
  applied: { label: "Applied", c: "#0F6B41" },
  partial: { label: "Partial", c: "#8A6212" },
  violated: { label: "Violated", c: "#B3241F" },
  na: { label: "N/A", c: "#6B665D" },
};
...
export default function AgenticWorkflow() {
  ...
  const Stat = ({ label, value, accent }) => (
    <div className="dark-card rounded-2xl p-5">
      <div className="num font-display text-3xl md:text-4xl font-black leading-none" style={{ color: accent || "#171512" }}>{value}</div>
  ...
  const PhasePill = ({ id, icon: Icon, label }) => {
    ...
    return (
      <div className="flex items-center gap-2.5 px-3.5 py-2 rounded-full transition-colors duration-300"
        ...
        <Icon size={15} style={{ color: active ? PINK : past ? "#15803D" : "#6B665D" }} />
        <span className="text-[12px] font-mono uppercase tracking-wider" style={{ color: active || past ? "#171512" : "#6B665D" }}>{label}</span>
      </div>
    );
  };

  return (
    <div className="cd" style={{ "--acc": ACCENT }} data-testid="agentic-workflow">
      ...
      <CaseTopBar accent={ACCENT} backTo="/ai-lab" backLabel="Back to AI Lab" />

      {/* ── HERO ───────────────────────────────────────────────────── */}
      <section className="cd-hero">
        <div className="cd-in">
          <p className="cd-eye">Agentic workflow · live demo</p>
          <h1 className="cd-h1">Point a swarm of AI agents at any website.</h1>
          <p className="cd-hero-deck">
            Drop in a URL and watch a fleet of specialist agents audit it in parallel, accessibility,
            content, hierarchy, IA, performance, then adversarially verify every finding, score it
            against Nielsen's 10 heuristics and the Laws of UX, and synthesize one prioritized
            report. This is how I design agentic systems.
          </p>
        </div>
      </section>
      ...
      <section className="cd-band cd-band-tight" ref={toolRef}>
        <div className="cd-in theme-light">
          ...
          <div className="dark-card rounded-3xl p-6 md:p-7 mb-9">
            <div className="flex flex-col lg:flex-row gap-4 lg:items-end">
              <label className="flex-1">
                ...
                <div className="flex items-center gap-2.5 rounded-xl px-3.5 py-3" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.12)" }}>
                  ...
                  <input value={url} onChange={(e) => setUrl(e.target.value)} onKeyDown={(e) => e.key === "Enter" && runCustom()}
                    ...
                    className="bg-transparent outline-none text-[#F4F3FA] text-sm w-full placeholder:text-[#8A8478]" />
                </div>
              </label>
              ...
              <button type="button" onClick={runCustom} disabled={running}
                ...
                style={{ background: running ? "rgba(255,255,255,0.06)" : PINK, color: running ? "#6B665D" : "#fff" }}>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2">
              ...
              <div className="flex items-center gap-2 text-[11px] text-[#A29CB4]">
                <span>try</span>
                {EXAMPLES.map((ex) => (
                  <button key={ex} type="button" onClick={() => setUrl(ex)} className="underline decoration-[#B9B2A5] hover:text-[#F4F3FA] transition-colors">{ex}</button>
                ))}
              </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
            ...
            <Stat label="Running in parallel" value={phase === "audit" ? 7 : phase === "idle" || phase === "scope" ? 0 : "✓"} accent="#171512" />
            <Stat label="Issues surfaced" value={rawFound} accent={PINK} />
            <Stat label="Confirmed after verify" value={preAudit ? "—" : confirmed} accent="#15803D" />
          </div>
            ...
            {(r ? r.agents : DIMS.map((d, i) => ({ label: d.label, scan: d.scan, findings: 0, start: 900 + i * 140, dur: 2400 }))).map((a) => {
              ...
              return (
                <div key={a.label} className="dark-card rounded-2xl p-4 transition-opacity duration-500"
                  style={{ opacity: started ? 1 : 0.4, borderLeft: `3px solid ${finished ? "#15803D" : started ? PINK : "rgba(255,255,255,0.1)"}` }}>
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="ml-auto text-[11px] font-mono tabular-nums" style={{ color: finished ? "#15803D" : "#6B665D" }}>
                  ...
                  <div className="mt-3 h-1.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.07)" }}>
                    <div className="h-full rounded-full" style={{ width: `${p * 100}%`, background: finished ? "#15803D" : PINK, transition: "width .1s linear" }} />
                  </div>
          ...
          {/* VERIFY */}
          <div className="mb-3 flex items-center gap-3">
            <span className="text-[11px] font-mono uppercase tracking-widest" style={{ color: phase === "verify" ? PINK : "#6B665D" }}>Adversarial verify · kill false positives</span>
            <span className="flex-1 h-px bg-white/10" />
          </div>
          <div className="dark-card rounded-3xl p-6 md:p-7 mb-10" style={{ opacity: !r || elapsed < T_AUDIT_END ? 0.4 : 1, transition: "opacity .5s" }}>
            <div className="flex flex-wrap items-center gap-x-8 gap-y-4">
              ...
              <div className="flex items-center gap-4 ml-auto font-display font-black">
                ...
                <div className="text-center">
                  <div className="num text-3xl md:text-4xl" style={{ color: "#15803D" }}>{preAudit ? "—" : confirmed}</div>
                  <div className="text-[9px] font-mono uppercase tracking-widest text-[#A29CB4] mt-1">confirmed</div>
                </div>
                <div className="text-center">
                  <div className="num text-xl md:text-2xl" style={{ color: "#B3241F" }}>{preAudit ? "—" : `-${Math.round(r.dropped * easeOut(verifyP))}`}</div>
          ...
          {/* SYNTHESIZE / REPORT */}
          <div className="mb-3 flex items-center gap-3">
            <span className="text-[11px] font-mono uppercase tracking-widest" style={{ color: showReport ? PINK : "#6B665D" }}>Synthesize · one prioritized report</span>
            <span className="flex-1 h-px bg-white/10" />
          </div>
          <div className="grid lg:grid-cols-5 gap-4" style={{ opacity: !r || elapsed < T_VERIFY_END ? 0.4 : 1, transition: "opacity .6s" }}>
            <div className="dark-card rounded-3xl p-6 lg:col-span-2">
              ...
              <div className="mt-4 space-y-3">
                {[{ k: "High", n: r ? r.sev.high : 0, c: "#B3241F" }, { k: "Medium", n: r ? r.sev.med : 0, c: "#8A6212" }, { k: "Low", n: r ? r.sev.low : 0, c: "#4896CF" }].map((s) => (
                  <div key={s.k}>
                ...
                {(r ? r.sample : []).map((f, i) => (
                  <div key={i} className="flex gap-3 items-start" style={{ opacity: showReport ? 1 : 0, transition: `opacity .4s ${i * 0.1}s` }}>
                    <span className="text-[9px] font-mono font-bold px-2 py-0.5 rounded mt-0.5 flex-shrink-0"
                      style={f.sev === "HIGH" ? { color: "#B3241F", background: "rgba(219,40,40,0.12)" } : f.sev === "MED" ? { color: "#8A6212", background: "rgba(232,180,49,0.12)" } : { color: "#4896CF", background: "rgba(72,150,207,0.12)" }}>{f.sev}</span>
                    <p className="text-[13px] leading-relaxed text-[#171512]">{f.t}</p>
                  </div>
                ))}
                {!r && <p className="text-[13px] text-[#A29CB4]">Run an audit to see the report.</p>}
              </div>
              <p className="mt-5 text-[10px] font-mono text-[#6B665D]">Demonstration · findings are illustrative of a real agent audit.</p>
            </div>
          </div>

          {/* HEURISTICS & UX-LAWS SCORECARD */}
          <div className="mt-10 mb-3 flex items-center gap-3">
            <span className="text-[11px] font-mono uppercase tracking-widest" style={{ color: showReport ? PINK : "#6B665D" }}>Scorecard · heuristics &amp; UX laws</span>
            <span className="flex-1 h-px bg-white/10" />
          </div>
          <div className="dark-card rounded-3xl p-6 md:p-7" style={{ opacity: showReport ? 1 : 0.4, transition: "opacity .6s" }}>
            <div className="flex flex-wrap items-baseline justify-between gap-3 mb-6">
              ...
              {r && (
                <div className="flex items-center gap-3 text-[11px] font-mono whitespace-nowrap">
                  <span style={{ color: "#15803D" }}>{r.scSummary.applied} applied</span>
                  <span style={{ color: "#8A6212" }}>{r.scSummary.improve} to improve</span>
                  {r.scSummary.na > 0 && <span style={{ color: "#6B665D" }}>{r.scSummary.na} n/a</span>}
                </div>
              )}
            </div>
            {[
            ...
            <p className="mt-5 text-[10px] font-mono text-[#6B665D]">Each row maps to a research-cited check (NN/g, WCAG, Fitts, Hick, Miller, Gestalt). Verdicts here are illustrative of a real audit.</p>
          </div>
        </div>
      </section>
      ...
      <section className="cd-band">
        <div className="cd-in">
          <Head eye="Why design it this way">The orchestration is the design decision.</Head>
          <div className="cd-3up">
            {[
              ...
              const I = p.icon;
              return (
                <div key={p.t} className="cd-tile">
                  <span className="w-11 h-11 rounded-xl grid place-items-center mb-5" style={{ background: "rgba(27,95,179,0.12)" }}><I size={20} style={{ color: ACCENT }} /></span>
                  <h3 className="cd-h3">{p.t}</h3>
                  <p>{p.d}</p>
                </div>
              );
            })}
          ...
          <div className="cd-aside">
            <h3 className="cd-h3">The point</h3>
            <p>
              I don&apos;t just use AI tools. I design the systems that make a fleet of agents fast,
              honest, and accountable to a human, and then I ship the result.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="cd-band cd-band-tight">
        <div className="cd-in">
          <Head eye="How I built this">Behind the build, in full transparency.</Head>
          <div className="cd-iters">
            {[
              ...
              <div key={s.n} className="cd-iter">
                <span className="cd-iter-n">{s.n}</span>
                <div>
          ...
          <p className="cd-note">
            The live version is a small serverless function that fetches a page and runs the audit
            with a real model, gated and cost-capped. Happy to walk through that architecture in
            person.
          </p>
        </div>
      </section>
      ...
      <Thanks
        items={NEXT_WORK}
        blurb="Want one pointed at your product? Happy to run it with you and walk through what it found."
      />
    </div>
  );
}
